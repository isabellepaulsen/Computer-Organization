 ...
 */
/**
 * Builds nodes and flows from the file using the constructors, and default values necessary.
 * 
 * @author andwolf
 * 
 */
public class WorkLoad extends WorkLoadDescription implements ReliabilityParameters {
  ...
  /**
 * Creates original array list of flows in the original order.
 */
private ArrayList<String> flowNamesInOriginalOrder = new ArrayList<>(); // array to hold names of
                                                                          // flows to preserve their
                                                                          // order
  /**
 * Acts as the array after the file has been "sorted" or put into priority order as need be.
 */
private ArrayList<String> flowNamesInPriorityOrder = new ArrayList<>();
  ...
  /**
   * Constructor for WorkLoad. Sets default parameters. Sets minPacketReceptionRate using file manager passed to object m.
   * Populates e2e's flows object as the input file is read. Calls WorkLoadListener, which reads 
   * input file, builds the AST of graph and the listener will build the node and flow data
   * objects
   * 
   * @param m used to set minPacketReceptionRate
   * @param e2e flows object is populated as input file read
   * @param inputFileName name of the input file
   */

  WorkLoad(Double m, Double e2e, String inputFileName) throws FileNotFoundException {
    super(inputFileName);
   ...
   * Populates e2e's flows object as the input file is read. Calls WorkLoadListener, which reads 
   * input file, builds the AST of graph and the listener will build the node and flow data
   * objects. Sets the numFaults based on parameter numFaults.
   * 
  ...
  WorkLoad(Integer numFaults, Double m, Double e2e, String inputFileName) throws FileNotFoundException {
    super(inputFileName);
    setDefaultParameters();
  ...
  /**
   * Adds a new flow node using name, priority, and index straight to the index.
   * 
 * @param flowName name of the flow to add
 */
public void addFlow(String flowName) {
   ...
   * 
 * @param flowName Passes the flow name.
 * @param nodeName Passes the node name.
 */
public void addNodeToFlow(String flowName, String nodeName) {
  ...
  /**
   * Returns the priority order of the node using the name of the flow and node name to index.
   * 
 * @param flowName Passes flow name.
 * @param nodeName Passes node name.
 * @return
 */
public Integer getFlowPriority(String flowName, String nodeName) {
  ...
  /**
   * Sets the deadline of a flow
   * 
 * @param flowName Passes flow name.
 * @param deadline Takes flow name and sets the deadline.
 */
public void setFlowDeadline(String flowName, Integer deadline) {
  ...
  /**
   * Returns the period of the node via the identifier.
   * 
 * @param flowName Passes the flow name as an identifier.
 * @return period of the node
 */
public Integer getFlowPeriod(String flowName) {
  ...
  /**
   * Returns the deadline of a flow.
   * 
   * @param flowName passes flowName
   * @return deadline deadline of a flow as an int
   */
  ...
  /**
   * Returns the phase of the node via the identifier.
   * 
 * @param flowName Passes the flow name as an identifier.
...
public Integer getFlowPhase(String flowName) {
  ...
  /**
   * Using flow name, returns back the number of TXs per link.
   * 
 * @param flowName Passes the flow name
 * @return number of TXs per link
 */
public Integer getFlowTxAttemptsPerLink(String flowName) {
  ...
  /**
   * Creates a list of flow objects from the FlowMap using the stream interface. Sorts by
   * a secondary key, the index. Sorts by primary key, the priority. Creates a new 
   * flowNamesInPriorityOrder that contains the flow names in the requested order.
   */
  ...
  /**
   * Creates a list of Flow objects from the FlowMap using the stream interface. Sorts
   * by a secondary key, the priority. Sorts by primary key, the deadline. Creates a new
   * flowNamesInPriorityOrder that contains the flow names in the requested order.
   */
  ...
  /**
   * Creates a list of Flow objects from the FlowMap using the stream interface. Sorts
   * by a secondary key, the priority. Sorts by primary key, the period. Creates a new
   * flowNamesInPriorityOrder that contains the flow names in the requested order.
   */
  ...
  /**
   * Returns an array list using the checked flow. 
   * The array list includes the maximum number of TXs and assumes the most edges.
   * 
 * @param flow Passes the flow.
 * @return txArrayList arraylist to return
 */
private ArrayList<Integer> getFixedTxPerLinkAndTotalTxCost(Flow flow) {
  ...
  /** 
   * Finds the number of tx attempts per link and total tx attempts.
   * 
   * @param flow passes flow
   * @param e2e passes e2e
   * @param M passes M
   * @param optimizationRequested passes optimizationRequested
   * @return number of tx attempts per link and total tx attempts in an arraylist
   */
  ...
  /**
   * Gets the names from the node map, sorts them, handles the case where the node names are
   * arrays of integers. Returns the node names sorted alphabetically.
   * 
   * @return nodeNames the names of the nodes sorted alphabetically
   */
   ...
   * the graph file
   * 
   * @return flow names in original order as an array
   */
  ...
  /**
   * Uses the node name to find node and then return the index (location) of such node.
   * 
 * @param nodeName Passes node name
 * @return
 */
public Integer getNodeIndex(String nodeName) {
  ...
  /**
   * Gets the flow node for the requested Flow, loops through the nodes in the flow to create
   * an array of the node names in the same order as they exist in the flow specification
   * (Graph file)
   * 
   * @param flowName passes flowName
   * @return node names in the order they exist in the flow specification
   */
  ...
  /**
   * Finds and returns the hyperPeriod, which is the least common multiple of all periods.
   * 
   * @return hyperPeriod least common multiple of all periods
   */
  ...
  /**
   * Determines the total tx attempts in the flow.
   * 
   * @param flowName name of the flow to count tx attempts
   * @return total tx attempts in the flow
   */
  ...
  /**
   * Returns an array of the number of transmission needed for each link (i.e. edge in the 
   * flow graph) to meet E2E target.
   * 
   * @param flowName passes flow
   * @return array of number of transmission needed for each link
   */
   ...
   * 
   * @param nodeName node to add edge to
  ...
  /**
   * Determines maximum flow length by iterating through the flow values.
   * 
   * @return max length of a flow
   */
